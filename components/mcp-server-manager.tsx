...
export const MCPServerManager = ({
  ...
  const addServer = () => {
    ...
    if ((newServer.type === 'sse' || newServer.type === 'streamable-http') && !newServer.url) {
      toast.error('Server URL is required for network transports');
      return;
    }
  ...
  const hasAdvancedConfig = (server: MCPServer) => {
    return (server.env && server.env.length > 0) || (server.headers && server.headers.length > 0);
  };
  ...
  const getServerDisplayUrl = (server: MCPServer): string => {
    ...
    return server.type === 'sse' || server.type === 'streamable-http'
      ? server.url
          ...
          <DialogDescription>
            ...
            {selectedServers.length > 0 && (
              <span className="block mt-1 text-xs font-medium text-primary">
                {selectedServers.length} server{selectedServers.length !== 1 ? 's' : ''} currently
                active
              </span>
            )}
          </DialogDescription>
            ...
            <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid gap-2 grid-cols-3">
                    ...
                    >
                      <Terminal
                        className={`h-5 w-5 shrink-0 ${
                          newServer.type === 'stdio' ? 'text-primary' : ''
                        }`}
                      />
                      <div>
                    ...
                    >
                      <Globe
                        className={`h-5 w-5 shrink-0 ${
                          newServer.type === 'streamable-http' ? 'text-primary' : ''
                        }`}
                      />
                      <div>
                        <p className="font-medium">HTTP</p>
                        <p className="text-xs text-muted-foreground">Streaming HTTP</p>
                      </div>
                    </button>
                  </div>
                </div>
              ...
              {newServer.type === 'sse' || newServer.type === 'streamable-http' ? (
                <div className="grid gap-1.5">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Full URL to the{' '}
                    {newServer.type === 'streamable-http' ? 'Streamable HTTP' : 'SSE'} endpoint of
                    the MCP server
                  </p>
                </div>
              ) : (
